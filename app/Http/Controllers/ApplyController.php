...
class ApplyController extends Controller
{
    ...
    public function masterApplyStore(MasterApplyRequest $request)
    {
        ...
        try {
            ...
            $add_images = $request->file('image');
            if( $add_images != NULL ) {
                // foreach ($add_images as $add_image) {
                    $add_image = new Master_applyimages;
                    $store_image = Storage::put('master-apply',  $add_images, 'public');
                    $add_image->image = 'https://s3.ap-northeast-2.amazonaws.com/immaster/' . $store_image;
                    $add_image->apply_id = $apply->id;
                    $add_image->save();
                // } 
            }         
